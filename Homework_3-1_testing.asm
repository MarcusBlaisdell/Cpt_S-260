...
shuffle:
  ...
  lb $t3, 0($t0)		# load first byte of string_1 into $t3
  beq $t3, $zero, printResult	# if we have reached the end of our string, skip store and jump to printResult
  sb $t3, 0($t2)		# store first byte of string_1 into first position of $t2, pointer to string_3
  addiu $t0, $t0, 1		# increment string_1
  lb $t3, 0($t1)		# load first byte of string_2 into $t3
  addiu $t2, $t2, 1		# increment address of $t2 by one, 
  sb $t3, 0($t2)		# store first byte of string_2 into second position of $t2
  addiu $t1, $t1, 1		# increment string_2
  addiu $t2, $t2, 1		# increment address of $t2 by one
